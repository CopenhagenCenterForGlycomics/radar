...
extern void openpl(), closepl();
extern void  linetype(int);
...
extern void clsline();
extern void xaxis(int n, char *title);
...
main (int argc, char *argv[]) {
  ...
  l1: fputs(iprompt1,stdout);
    fflush(stdout);
    if (fgets(fname,sizeof(fname),stdin)==NULL) exit(1);
  ...
  gettitle(fname,title,sizeof(title));

  if (argc > 2) {
    sscanf(argv[2],"%d",&wind);
    if (wind < 2 || wind > 21) wind = 7;
  }
  else wind = 7;
  mid = wind/2;
...
void
makemap(char *input, int *map, int n)
{
  int i;

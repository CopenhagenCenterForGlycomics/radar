...
discons(char *seqc0, char *seqc1, int nc)
{
  ...
  for (il=0; il<(nc+llen-1)/llen; il++) {
	...
	cline[0][i+8]=' ';
      ...
      lloff = sq1off-1 + (long)(ioff1-del1);
      if (cl1 && lloff%10 == 9)  {
	...
	if (toupper(line[0][i])==toupper(line[2][i]) || ((dnaseq==1) && (
...
void
disgraph(int n0, int n1, float percent, int score,
...
void 
r_memcpy(char *dest, char *src, int cnt)
{
  while (cnt--) *dest++ = *src++;
}

void
l_memcpy(char *dest, char *src, int cnt)
{
  dest = dest+cnt;
  src = src+cnt;
  while (cnt--) *--dest = *--src;

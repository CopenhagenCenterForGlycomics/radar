...
char *verstr="version 2.1u09 December 2006";
char *progstr="LALIGN";
...
extern void revcomp(unsigned char *seq, int n);
...
#ifdef TPLOT
extern void openplt(long, long, int, int);
void drawdiag(long n0, long n1);
void closeplt();
#endif

extern void SIM(const unsigned char *A,
...
main(int argc, char **argv) {
  ...
  char tname[QFILE_SIZE], lname[QFILE_SIZE];
  int m_score;
  int i;
  int match, mismh;
  int sq0save;
  ...
  l2:	fputs(iprompt2,stderr);
    ...
    fprintf(stderr," E(limit) [%g]: ",e_val);
    fflush(stderr);
...
#ifdef TPLOT
  openplt((long)n0,(long)n1, sq0off,sq1off);
